...
<script lang="ts">
	...
	export let formatTitle = (d: string) => d;

	/** A function to format the series name. */
	export let formatKey = (d: string) => titleCase(d);

	/** A function to format the value. */
	export let formatValue = (d: number) => (isNaN(+d) ? d : commas(d));
	...
	export let dataset: Array<any> | undefined = undefined;
